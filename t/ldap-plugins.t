...
is( $c[0]{args}{group} // ( $c[0]{entry_dn} =~ /^cn=([^,]+)/ )[0], 'bob', 'delete hook saw the cascaded group' );
...
( $ret, $err )
	= pt_try( sub { $pt->plugin( { ldap => $ldap, entry => $entry, do => 'pluginMulti' }, { tag => 'x' } ) } );
is( $err,                            '',            'direct plugin() call succeeds' );
is( scalar @NisabaTestPlugin::CALLS, $n_before + 2, 'comma-separated hook ran the plugin list (both entries)' );
...
( $ret, $err ) = pt_try( sub { $pt->plugin( { ldap => $ldap, entry => 'not-an-entry', do => 'pluginMulti' }, {} ) } );
is( $pt->error, 43, 'non-entry sets error 43 (entryNotLDAPEntry)' );

( $ret, $err ) = pt_try( sub { $pt->plugin( { ldap => 'not-ldap', entry => $entry, do => 'pluginMulti' }, {} ) } );
is( $pt->error, 44, 'non-LDAP object sets error 44 (ldapNotLDAP)' );

( $ret, $err ) = pt_try( sub { $pt->plugin( { ldap => $ldap, entry => $entry, do => 'pluginNotConfigured' }, {} ) } );
is( $pt->error, 40, 'unconfigured hook sets error 40 (pluginConfigMissing)' );

( $ret, $err ) = pt_try( sub { $pt->plugin( { ldap => $ldap, entry => $entry, do => 'pluginBogus' }, {} ) } );
...
SKIP: {
	...
	do {
		...
		open STDOUT, '>', \$out;
		( $ret, $err )
			= pt_try( sub { $pt->plugin( { ldap => $ldap, entry => $entry, do => 'pluginDump' }, { k => 'v' } ) } );
	};
