...
$t->get_ok('/users')->status_is(200)->content_like( qr/LDAP down/, 'index shows flash error on subsequent request' );

# ── add (GET) ─────────────────────────────────────────────────────────────────

$t->get_ok('/users/add')->status_is(200)->content_like( qr/Add User/, 'add form renders' );
...
my $gecos_args;
_install_stubs( $t->app, userGECOSchange => sub { my ( $self, $args ) = @_; $gecos_args = $args }, );
...
my $shell_args;
_install_stubs( $t->app, userShellChange => sub { my ( $self, $args ) = @_; $shell_args = $args }, );
...
my $uid_args;
_install_stubs( $t->app, userUIDchange => sub { my ( $self, $args ) = @_; $uid_args = $args }, );
...
my $gid_args;
_install_stubs( $t->app, userGIDchange => sub { my ( $self, $args ) = @_; $gid_args = $args }, );
...
my $home_args;
_install_stubs( $t->app, userHomeChange => sub { my ( $self, $args ) = @_; $home_args = $args }, );
	...
	)
{
	...
	my $captured;
	_install_stubs( $t->app, $method => sub { my ( $self, $args ) = @_; $captured = $args }, );
...
} ## end for my $test ( [ 'title', 'userTitleChange'...])
	...
	)
{
	...
	is( $captured->{$field}, $value, "$method got correct $field" );
} ## end for my $test ( [ 'mail_add', 'userMailAdd',...])

# ── update: SSH key add/remove ────────────────────────────────────────────────

my $sshkey_args;
_install_stubs( $t->app, userSSHPublicKeyAdd => sub { my ( $self, $args ) = @_; $sshkey_args = $args }, );
...
my $sshkey_rm_args;
_install_stubs( $t->app, userSSHPublicKeyRemove => sub { my ( $self, $args ) = @_; $sshkey_rm_args = $args }, );
	...
	)
{
	...
	$t->post_ok( '/users/alice', form => { action => $action, $field => $value } )
		...
		->header_like( Location => qr{/users/alice}, "$action redirects to show" );
	is( $captured->{$field}, $value, "$method got correct $field" );
} ## end for my $test ( [ 'totp_status', 'userTotpStatusSet'...])

# TOTP secret remove
my $totp_rm_called;
_install_stubs( $t->app, userTotpSecretRemove => sub { $totp_rm_called = 1 }, );
...
_install_stubs( $t->app, userTotpEnrolledDateSet => sub { $totp_enrolled_called = 1 }, );
...
$t->get_ok('/users/alice')->status_is(200)->content_like( qr/disabled/, 'totp_scratch_add shows disabled message' );
...
my $delete_args;
_install_stubs( $t->app, deleteUser => sub { my ( $self, $args ) = @_; $delete_args = $args }, );
...
my $pass_args;
_install_stubs( $t->app, userSetPass => sub { my ( $self, $args ) = @_; $pass_args = $args }, );
...
my $rmpass_called;
_install_stubs( $t->app, userRemovePassword => sub { $rmpass_called = 1 }, );
...
my $iop_called;
_install_stubs( $t->app, userConvertToInetOrgPerson => sub { $iop_called = 1 }, );
...
my $lpk_called;
_install_stubs( $t->app, userConvertToLdapPublicKey => sub { $lpk_called = 1 }, );
$t->post_ok('/users/alice/lpk')->status_is(302)->header_like( Location => qr{/users/alice}, 'lpk redirects to show' );
...
my $totp_called;
_install_stubs( $t->app, userConvertToTotp => sub { $totp_called = 1 }, );
...
my $totp_gen_called;
_install_stubs( $t->app, userTotpGenerateSecret => sub { $totp_gen_called = 1; return 'NEWSECRET' }, );
...
my $passkey_called;
_install_stubs( $t->app, userConvertToPasskeyUser => sub { $passkey_called = 1 }, );
...
my $passkey_rm_args;
_install_stubs( $t->app, userPasskeyCredentialRemove => sub { my ( $self, $args ) = @_; $passkey_rm_args = $args }, );
...
my @group_add_calls;
_install_stubs( $t->app, groupAddUser => sub { my ( $self, $args ) = @_; push @group_add_calls, $args }, );
...
my $rmgrp_args;
_install_stubs( $t->app, groupRemoveUser => sub { my ( $self, $args ) = @_; $rmgrp_args = $args }, );
