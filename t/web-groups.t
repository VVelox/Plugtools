...
$t->get_ok('/groups')->status_is(200)->content_like( qr/LDAP down/, 'index shows flash error on subsequent request' );

# ── add (GET) ─────────────────────────────────────────────────────────────────

$t->get_ok('/groups/add')->status_is(200)->content_like( qr/Add Group/, 'add form renders' );
...
_install_stubs( $t->app );
$t->post_ok('/groups/clean')->status_is(302)->header_like( Location => qr{/groups$}, 'clean redirects to index' );
...
$t->get_ok('/groups/admins')->status_is(200)->content_like( qr/bob/, 'show renders non-member in dropdown' );
...
my $gid_args;
_install_stubs( $t->app, groupGIDchange => sub { my ( $self, $args ) = @_; $gid_args = $args }, );
...
my $desc_args;
_install_stubs( $t->app, groupDescriptionChange => sub { my ( $self, $args ) = @_; $desc_args = $args }, );
...
my $default_gid_args;
_install_stubs( $t->app, groupGIDchange => sub { my ( $self, $args ) = @_; $default_gid_args = $args }, );
...
my $deleted;
_install_stubs( $t->app, deleteGroup => sub { my ( $self, $group ) = @_; $deleted = $group }, );
...
my $add_member_args;
_install_stubs( $t->app, groupAddUser => sub { my ( $self, $args ) = @_; $add_member_args = $args }, );
...
my $rm_member_args;
_install_stubs( $t->app, groupRemoveUser => sub { my ( $self, $args ) = @_; $rm_member_args = $args }, );
