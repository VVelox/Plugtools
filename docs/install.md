...
each running under `daemon(8)` as `www` with syslog output. Copy them
into `/usr/local/etc/rc.d/` and in `rc.conf`...
...
run `prefork`, restart on failure, read optional overrides from
`/usr/local/etc/nisaba/<name>.env`, and are hardened
(`ProtectSystem=strict` with only `/var/db/nisaba` writable,
`MemoryDenyWriteExecute`, syscall filtering).
...
TLS-terminating reverse proxy (or be given certs directly via
`-l 'https://*:8443?cert=...&key=...'`). If the proxy speaks plain HTTP
to the app *and* the app is reached over plain HTTP by browsers, set
`cookieSecure=0`; otherwise leave it alone. Behind a proxy also set
`passkeyRpId` and (for the SSO) `ssoIssuer` explicitly — see
